<template >
    <div>
        <div class="row" id="container">
            <div class=" col-md-12" id="main" :setKey="selectedChannelName"> </div>
            <ResistanceChart class="col-md-12" ></ResistanceChart>
        </div>
    </div>
</template>
...
import ResistanceChart from "./ResistanceChart.vue"
...
export default {
    ...
    props: ['selectedChannelData','selectedChannelName','keyInCurveShow'],
    ...
    watch: {
        selectedChannelData: {
            ...
            handler(val) {
                ...
                var echartData = new EchartObj(this.myChart)
                echartData.title ={ text: this.selectedChannelName}
                echartData.drawChart(val)
            }
        },
    }
}

function EchartObj(myChart) {
    ...
    this.title = {}
    this.dataZoom = []
    this.axisPointer = { link: [] }
    // this.tdmsInfo = tdmsInfo
    this.frequencySeries = []
    this.option;
}

EchartObj.prototype.drawChart = function (channel) {
    // if (!this.tdmsInfo) { alert("请先载入数据文件！"); return false }
    this.getChartSeries(channel);
    // console.log(this)
        ...
        tooltip: {
            ...
            axisPointer: {
                ...
                label: {
                    ...
                    label: { show: true, fontSize: 16, formatter: function (param) {  return (param.value[1] + 'ms') } },
                }
            },
    ...
    this.myChart.on('click', 'series.line', (param) => {           //
        // console.log(param)
...
EchartObj.prototype.getChartSeries = function (channel) {
    // echartData = { grids: [], xAxes: [], yAxes: [], series: [], titles: [], x_Axis_data: [] };
    var allItems = []
    //contactHeight: 触头分合闸曲线高度， travelHeight: 行程曲线高度, coilHeight: 分合闸线圈曲线高度， motorHeight: 电机电流曲线高度
    var contactHeight = 1, travelHeight = 44, coilHeight = 10, sGap = 1, bGap = 4, topPosion = 0;
    var signalSequence = [contactHeight, contactHeight, contactHeight, travelHeight, coilHeight, coilHeight, coilHeight]
    // this.title = {  }
    channel.forEach(function (v, i) {
        var data = [];
        allItems.push(v.name)
        if (v.data) {
            // this.basicFrequency > 0 ? 0 : this.basicIncrement = parseInt(1 / v.wf_increment)

            var heightTop = ((i) => {
                topPosion = topPosion + (i === 0 ? 0 : signalSequence[i - 1]) + ((i != 0 & i < 3) ? sGap : bGap)
                // var h = i===0 ? contactHeight  : hArr[i]
            ...
            this.yAxes.push({
                ...
                min: (i < 3) ? 0 : function (value) {
                            return Math.round((value.min)*0.98);
                         },
                max: (i < 3) ? 1 : function (value) {
            ...
            var freq = parseInt(1 / v.wf_increment)
            ...
            if (freIsExist < 0) {
                ...
                this.axisPointer.link.push({ xAxisIndex: [i] })
                this.dataZoom.push({ type: 'inside', xAxisIndex: [i], filterMode: 'filter', })
            } else {
                this.dataZoom[freIsExist].xAxisIndex.push(i)
                this.axisPointer.link[freIsExist].xAxisIndex.push(i)
            }
            this.series.push({
